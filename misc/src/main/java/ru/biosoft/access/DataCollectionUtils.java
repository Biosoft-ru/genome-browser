...
public class DataCollectionUtils
{
     ...
     * @deprecated this method not safe and will be removed in future. The
     * following alternatives are available: - Try to avoid using this method if
     * it's not really necessary - Use methods from this class for common
     * actions like (getFile, getSqlConnection, isFileAccepted,
     * checkPrimaryElementType and so on) - Use method
     * fetchPrimaryCollectionPrivileged in privileged code section (see
     * SecurityManager.runPrivilegedAction)
     */
    @Deprecated public static DataCollection fetchPrimaryCollection(DataCollection<?> parent, int access)
    {
     ...
     * actions like (getFile, getSqlConnection, isFileAccepted,
     * checkPrimaryElementType and so on) - Use method
     * fetchPrimaryElementPrivileged in privileged code section (see
     * SecurityManager.runPrivilegedAction)
     */
    @Deprecated public static DataElement fetchPrimaryElement(DataElement parent, int access)
    {
    ...
    /**
     * Returns type of specified element This is fast check, so note that it
     * might not be exact type, but best guess. Also element may not exist at
     * all.
     */
     ...
     * @return
     */
    @SuppressWarnings("unchecked") public static String getClassTitle(Class<? extends DataElement> clazz)
    {
        if( clazz == null )
    ...
    /**
     * Returns file for new FileDataElement Consider refactoring
     */
    ...
    /**
     * Checks whether file is accepted by specified FileCollection Consider
     * refactoring
     */
    ...
    /**
     * Returns SqlConnectionHolder connection Consider refactoring
     */
    ...
    /**
     * Copy all properties starting with 'persistent.' prefix from origin to
     * collection
     */
    ...
    /**
     * Copy all properties starting with 'analysis.' prefix from origin to
     * collection and change path from source to target if any.
     * 
     * @todo this method should be moved somewhere to reuse constants from
     * AnalysisParametersFactory
     */
     ...
     * @return java.util.Collection<T> view for given
        ...
        if( path.optParentCollection() == null )
        {
            ...
            if( parentPath.isEmpty() )
            {
                if( parentPaths.isEmpty() )
                    throw new DataElementCreateException(new MissingParameterException("Path"), path.getParentPath(), FolderCollection.class);
                throw new DataElementCreateException(new DataElementNotFoundException(parentPaths.get(0)), path.getParentPath(), FolderCollection.class);
            }
            if( !(checkPrimaryElementType(parentPath.optDataElement(), FolderCollection.class)) )
                throw new DataElementCreateException(new DataElementInvalidTypeException(parentPath, FolderCollection.class), path.getParentPath(), FolderCollection.class);
    ...
    /**
     * Checks whether collection in given path has not exceeded its quota (if
     * applicable)
     * 
