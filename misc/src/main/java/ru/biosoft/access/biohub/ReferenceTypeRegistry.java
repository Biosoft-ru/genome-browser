...
public class ReferenceTypeRegistry implements Iterable<ReferenceType>
{
    /**
    ...
    protected volatile List<ReferenceType> referenceTypes;
    protected volatile Map<String, ReferenceType> nameToReferenceType;
    ...
    protected final void init()
    {
        if( !initialized )
        {
            synchronized (this)
            {
                ...
                {
                    if( initializing )
                        throw new InitializationException("Concurrent initialization of ReferenceTypeRegistry");
                    //throw new InitializationException("Concurrent initialization of ReferenceTypeRegistry point " + extensionPointId);
                    initializing = true;
                    try
                    {
                        //loadExtensions();
                        ...
                        nameToReferenceType = new HashMap<>();
                    }
                    finally
                    {
                        initializing = false;
    ...
    @Override public Iterator<ReferenceType> iterator()
    {
        init();
        return Collections.unmodifiableList(referenceTypes).iterator();
    }
    
    public StreamEx<ReferenceType> stream()
    {
        init();
        return StreamEx.of(referenceTypes);
    }

    public StreamEx<String> names()
    {
        init();
        return StreamEx.ofKeys(nameToReferenceType);
    }

    public EntryStream<String, ReferenceType> entries()
    {
        init();
        return EntryStream.of(nameToReferenceType);
    }

    public void registerReferenceType(ReferenceType type, String name)
    {
        registerReferenceType(type, name, null);
    }

    public void registerReferenceType(ReferenceType type, String name, String urlTemplate)
    {
        Class clazz = type.getClass();
        if( clazz.equals(defaultType.getClass()) )
            type = defaultType;
        if( type instanceof ReferenceTypeSupport )
        {
            ((ReferenceTypeSupport) type).setUrlTemplate(urlTemplate);
        }
        referenceTypes.add(type);
        nameToReferenceType.put(name, type);
    }
    ...
    protected void loadCollectionReferenceTypes(DataCollection<DataCollection<?>> repository)
    {
        ...
        {
            @Override public void elementAdded(DataCollectionEvent e) throws Exception
            {
            ...
            @Override public void elementWillRemove(DataCollectionEvent e) throws DataCollectionVetoException, Exception
            {
                DataElement de = e.getDataElement();
                if( de instanceof DataCollection )
                {
    ...
    protected void addDataCollectionType(DataCollection<?> dc) throws Exception
    {
        ...
        ExProperties.getSubProperties(dc.getInfo().getProperties(), ReferenceType.REFERENCE_TYPE_PROPERTY).forEach((key, props) -> {
    ...
    /**
     * Returns default reference type If several default reference type are
     * available, any of them may be returned.
     * 
    ...
    /**
     * Lookups reference type by given display name first, if nothing found, try
     * to lookup by stable name
     * 
    ...
    public static @CheckForNull ReferenceType optReferenceType(String name)
    {
        if( name == null || name.isEmpty() )
            return null;
        if( getDefaultReferenceType().getDisplayName().equals(name) || getDefaultReferenceType().getStableName().equals(name) )
    ...
    {
        return instance.stream().findAny(type -> type.getClass().equals(typeClass)).orElseThrow(() -> new InternalException("Reference type is not registered: " + typeClass));
    }
    ...
    /**
     * Lookups list of reference types by given classes This method is handy for
     * BioHub methods like getSupportedInputTypes and getSupportedMatching
     * 
     * @param typeClasses list of classes of reference type
     * @return array of found reference types or empty array if no reference
     * types found
     */
    @SafeVarargs public static @Nonnull ReferenceType[] getReferenceTypes(Class<? extends ReferenceType>... typeClasses)
    {
     ...
     * @return ReferenceType reference type of this
     * ru.biosoft.access.core.DataCollection. If no reference type was specified
     * explicitly, default reference type will be returned (or null if no
     * default reference type applicable).
     */
    ...
    /**
     * Returns reference type of ru.biosoft.access.core.DataElement by given
     * element path. This method doesn't instantiate
     * ru.biosoft.access.core.DataElement and works only for elements inside
     * GenericDataCollection
     * 
     * @param path path to ru.biosoft.access.core.DataElement to fetch reference
     * type from
     * @return ReferenceType reference type of specified
     * ru.biosoft.access.core.DataElement. If no reference type was specified
     * explicitly, default reference type will be returned (or null if no
     * default reference type applicable).
     */
    ...
    /**
     * Sets reference type for ru.biosoft.access.core.DataCollection Note: this
     * method doesn't check whether reference type is applicable for given
     * ru.biosoft.access.core.DataCollection
     * 
     ...
     * @param baseType allows you to limit search to sub-hierarchy of types with
    ...
    {
        return types().filter(baseType::isInstance).maxBy(type -> StreamEx.of(references).filter(TextUtil::nonEmpty).mapToInt(type::getIdScore).sum())
