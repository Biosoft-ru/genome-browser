...
public class WebActionsProvider extends WebProviderSupport
{
    ...
    DynamicActionFactory.dynamicActions()
            .map( action ->
            new JSONObject()
                    .put("id", action.getTitle())
                    .put("label", getLabel( action ))
                    .put("icon", "../biouml/web/action?type=dynamic&action=toolbar_icon&name=" + action.getTitle().replaceAll(" ", "%20"))
                    .put("numSelected", action.getNumSelected())
                    .put("acceptReadOnly", action.isAcceptReadOnly())
    ...
    public static JSONArray getVisibleActions(Object model)
    {
        ...
        DynamicActionFactory.dynamicActions().filter( action -> perspective.isActionAvailable( action.getTitle() ) )
            ...
            .forEach( action -> {
                if( !result.isEmpty()
                            && !DynamicActionFactory.getDynamicAction(result.getString(result.length() - 1)).isSameGroup(action) )
                        result.put("");
