...
public class PerspectivesProvider extends WebJSONProviderSupport
{
    ...
    public void process(BiosoftWebRequest arguments, JSONResponse response) throws Exception
    {
        ...
        {
            perspectiveName = ServerPreferences.getPreferences().getStringValue("Perspective", availablePerspectives.iterator().next());
        }
        else
        {
            ServerPreferences.getPreferences().addValue("Perspective", perspectiveName);
            ignore = ignoreIfSame;
        }
